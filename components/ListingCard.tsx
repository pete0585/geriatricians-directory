...
export default function ListingCard({ listing }: ListingCardProps) {
    ...
    return (<Link href={`/listings/${listing.slug}`} className={`card block hover:shadow-card-hover transition-shadow duration-200 overflow-hidden ${isFeatured ? 'ring-2 ring-gold' : ''}`}>
      {null}
      ...
      <div className="p-5">
        <div className="flex items-start justify-between gap-3 mb-3">
          <div className="flex-1 min-w-0">
            ...
            {listing.practice_name && (<p className="text-sm text-navy-400 mt-0.5 truncate">{listing.practice_name}</p>)}
          </div>
          {listing.photo_url && (<div className="w-12 h-12 rounded-full overflow-hidden flex-shrink-0 bg-navy-100">
        ...
        {/* Subspecialties */}
        {listing.subspecialties?.length > 0 && (<div className="flex flex-wrap gap-1 mb-3">
            {listing.subspecialties.slice(0, 3).map((s) => (<span key={s} className="inline-block px-2 py-0.5 bg-navy-50 text-navy-600 text-xs rounded-full">
                {getSubspecialtyLabel(s)}
              </span>))}
          </div>)}

        {listing.bio && (<p className="text-sm text-navy-400 leading-relaxed mb-3">
        ...
        <div className="flex items-center gap-4 pt-3 border-t border-navy-100 text-xs text-navy-400">
          {listing.phone && (<span className="flex items-center gap-1">
              <Phone className="w-3 h-3" aria-label="phone"/>
              {listing.phone}
            </span>)}
          {listing.website && (<span className="flex items-center gap-1">
              <Globe className="w-3 h-3" aria-label="website"/>
              Website
            </span>)}
          {!isClaimed && (<span className="text-navy-300 italic">Unclaimed listing</span>)}
        </div>
      </div>
    </Link>);
}
