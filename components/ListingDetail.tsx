        ...
        <div className="lg:col-span-2 space-y-6">
          <div className="card p-6">
            ...
            <div className="flex items-start gap-4 mb-4">
              {listing.photo_url ? (<div className="w-20 h-20 rounded-full overflow-hidden flex-shrink-0 bg-navy-100">
                  {/* eslint-disable-next-line @next/next/no-img-element */}
                  <img src={listing.photo_url} alt={listing.full_name} className="w-full h-full object-cover"/>
                </div>) : (<div className="w-20 h-20 rounded-full bg-navy-100 flex-shrink-0 flex items-center justify-center">
              ...
              <div>
                ...
                {listing.practice_name && (<p className="text-navy-500 mt-1">{listing.practice_name}</p>)}
            ...
            <div className="flex flex-wrap gap-2 mb-4">
              {listing.is_abim_certified && (<span className="badge-abim">
                  <Award className="w-3.5 h-3.5" aria-label="ABIM"/>
                  Board-Certified by ABIM
                </span>)}
              {listing.is_ags_member && (<span className="badge bg-purple-50 text-purple-700">
                  <Shield className="w-3.5 h-3.5" aria-label="AGS"/>
                  AGS Member
                </span>)}
              {null}
              {listing.is_accepting_new_patients === true && (<span className="badge-accepting">
                  <UserCheck className="w-3.5 h-3.5" aria-label="accepting"/>
                  Accepting New Patients
                </span>)}
              {listing.is_accepting_new_patients === false && (<span className="badge bg-red-50 text-red-600">
                  Not Accepting New Patients
                </span>)}
              {listing.offers_telehealth && (<span className="badge-telehealth">
          ...
          {/* Specialties */}
          {listing.subspecialties?.length > 0 && (<div className="card p-6">
              <h2 className="font-display font-semibold text-navy text-base mb-3">Specialty Areas</h2>
              <div className="flex flex-wrap gap-2">
                {listing.subspecialties.map((s) => (<span key={s} className="inline-block px-3 py-1.5 bg-navy-50 text-navy-700 text-sm rounded-full font-medium">
          ...
          {/* Conditions treated */}
          {listing.conditions_treated?.length > 0 && (<div className="card p-6">
              <h2 className="font-display font-semibold text-navy text-base mb-3">Conditions Treated</h2>
              <div className="flex flex-wrap gap-2">
                {listing.conditions_treated.map((c) => (<span key={c} className="inline-block px-3 py-1.5 bg-sage-50 text-sage-700 text-sm rounded-full">
          ...
          {/* Insurance */}
          {listing.insurance_accepted?.length > 0 && (<div className="card p-6">
              <h2 className="font-display font-semibold text-navy text-base mb-3">Insurance Accepted</h2>
              <div className="flex flex-wrap gap-2">
                {listing.insurance_accepted.map((ins) => (<span key={ins} className="inline-block px-3 py-1.5 bg-gray-50 text-gray-700 text-sm rounded-full border border-gray-200">
        ...
        <div className="space-y-5">
          ...
          <div className="card p-5">
            ...
            {(<div className="space-y-3">
                {listing.address_line1 && (<div className="flex items-start gap-2 text-sm text-navy-600">
                ...
                {listing.phone && (<a href={`tel:${listing.phone}`} className="flex items-center gap-2 text-sm text-navy hover:text-navy-700">
                    <Phone className="w-4 h-4 text-navy-400" aria-label="phone"/>
                    {listing.phone}
                  </a>)}
                {listing.email && (<a href={`mailto:${listing.email}`} className="flex items-center gap-2 text-sm text-navy hover:text-navy-700 break-all">
                    <Mail className="w-4 h-4 text-navy-400 flex-shrink-0" aria-label="email"/>
                    {listing.email}
                  </a>)}
                {listing.website && (<a href={listing.website} target="_blank" rel="noopener noreferrer" className="flex items-center gap-2 text-sm text-navy hover:text-navy-700">
                    <Globe className="w-4 h-4 text-navy-400" aria-label="website"/>
                    Visit Website
                  </a>)}
              </div>)}
          </div>
          {!isClaimed && (<div className="card p-5 bg-navy-50 border-navy-200">
          ...
          {/* NPI info */}
          {listing.npi && (<div className="card p-5">
              ...
              <div className="text-xs text-navy-500 space-y-1">
                ...
                {listing.credentials && (<div className="flex justify-between">
